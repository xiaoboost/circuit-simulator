...
import { Observable, map, from, fromEvent } from 'rxjs';
// import { DrawEvent, DrawEventData } from './types';
// import { sheetEl, current, setSheetElement } from './store';
import { Map } from '../../../store';

let sheetElObservable: Observable<MouseEvent> | undefined;

/** 事件总线初始化 */
export function useMouseBusInit({ current: sheetEl }: RefObject<HTMLElement>) {
  const [mapState] = useWatcher(Map.state);
  const currentEnterPart = useRef<string>();

  function enterPart(id: string) {
    if (!sheetEl) {
      return;
    }
  }

  function leavePart(id: string) {
    if (!sheetEl) {
      return;
    }
  }

  function enterLine(id: string) {
    if (!sheetEl) {
      return;
    }
  }

  function leaveLine(id: string) {
    if (!sheetEl) {
      return;
    }
  }
