...
class Point {
     ...
     * @param {[number, number] | number | Point} start
     * @param {[number, number] | number | Point} [end]
     */
     ...
     *
     * @param {[number, number] | number | Point} added
     ...
     *
     * @param {[number, number] | number | Point} multiplier
     * @param {number} [label=1]
     * @returns {Point}
     */
    mul(multiplier, label = 1) {
        const sum = new Point(0);
     ...
     * 是否在线段内
     *
     * @param {[number, number] | Point[]} segment
     * @returns {boolean}
     */
    isInLine(segment) {
     ...
     *
     * @param {(point: [number, number] | Point) => boolean} predicate
     * @param {number} [factor=1]
     * @returns {Point[]}
     */
    aroundInf(predicate, factor = 1) {
        const ans = predicate(this) ? [new Point(this)] : [];
     ...
     *
     * @param {[number, number] | Point[]} points
 ...
 *
 * @param {[number, number] | number | Point} [start=[0, 0]]
 * @param {[number, number] | number | Point} [end]
 * @returns {Point}
 */
function $P(start = [0, 0], end) {
