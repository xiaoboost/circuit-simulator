...
describe('point.ts: class of point and vector', () => {
    test('create a instance', () => {
        ...
        expect(Point.from(5)).toEqual($P([5, 5]));
    });
    ...
    test('Point.prototype.isParallel()', () => {
        expect($P(0).isParallel([-1, 2])).toBeTrue();
        expect($P(0).isParallel([2, 10])).toBeTrue();
    ...
    test('Point.prototype.isSameDirection()', () => {
        ...
        // 零向量和任何向量都同向
        expect($P(0).isSameDirection([-1, 0])).toBeTrue();
        expect($P(0).isSameDirection([0, 0])).toBeTrue();
    });
    test('Point.prototype.isOppoDirection()', () => {
        ...
        // 零向量和任何向量都反向
        expect($P(0).isOppoDirection([-1, 0])).toBeTrue();
        expect($P(0).isOppoDirection([0, 0])).toBeTrue();
    });
    ...
    test('Point.prototype.closest()', () => {
        expect(() => $P(0).closest([])).toThrowError('(point) points can not be a empty array.');
        expect($P(0).closest([[1, 2], [0.5, 0.4], [-10, 50], [0.3, -0.8], [-1, -0.1]])).toEqual($P(0.5, 0.4));
    });
    test('Point.prototype.minAngle()', () => {
        expect(() => $P(0).minAngle([])).toThrowError('(point) vectors can not be a empty array.');
